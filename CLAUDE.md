...
- **UX unificado + resilience + deploy prod:** 0 `<select>` nativos, 0 `<tbody>` raw. `WidgetErrorBoundary` en todos los componentes data-rendering (5 nuevos). Drawer con slide animation sin backdrop. Dashboard empty state + toggles disabled sin data. Border radius estandarizado (`rounded-lg`). Tenant CRUD completo. 260 tests / 34 suites. Backend+frontend deployed. [CHANGELOG — 2.6.0-alpha.0](CHANGELOG.md)
