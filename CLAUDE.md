...
- **UX unificado + tests completos:** `ChartSkeleton` en 6 páginas. 18 `<select>` → `DropdownSelect` (0 restantes). `TableStateBody` en todas las tablas (0 `<tbody>` raw). Forms Modal → Drawer. 8 tests UI + 3 suites integración (login, CRUD buildings, cross-tenant). 238 tests / 29 suites. Buildings columna "Empresa", Meters columna "Edificio". Globe Power excluido de KPIs. Vite proxy → AWS. [CHANGELOG — 2.6.0-alpha.0](CHANGELOG.md)
