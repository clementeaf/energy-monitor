...
- **2.44.0:** UX pass + mapa indoor enriquecido. **Observabilidad:** 7 KPIs en una fila (`grid-cols-7`), sección ingestión eliminada, KPI p95 duplicado removido, componentes flex-wrap, alertas `flex-1` scroll interno. **Selectores:** 24 `<select>` nativos migrados a `DropdownSelect` en 9 archivos (zero nativos restantes). **Layout:** `max-h-[70vh]` y `max-h-[calc(...)]` eliminados de 28 páginas (tablas crecen natural, scroll via layout wrapper). UsersPage flex layout con tabla scroll interno. **Mapa Indoor:** fix tiles URL (`localhost:4000` → `window.location.origin`, funciona con proxy Vite). Popup enriquecido: al seleccionar tienda matchea store→tenant unit por nombre, muestra kWh + medidores + última factura si hay datos, link "Ver detalle →" navega a vista medidor (SPA). `SelectedPoint.extraHtml` + `MapView.onNavigate` para popup navigation. [CHANGELOG — 2.44.0-alpha.0](CHANGELOG.md)
