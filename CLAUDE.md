...
- **Chart skeletons + Platform Dashboard UX + Buildings cross-tenant:** `ChartSkeleton` compartido (SVG shimmer) en 6 páginas. `Chart` acepta `loading` prop. PlatformDashboard KPIs compactos (7 en fila), Globe Power excluido. Buildings: columna "Empresa" en cross-tenant, form migrado a Drawer con selector empresa (DropdownSelect), dirección obligatoria. Vite proxy → AWS backend. [CHANGELOG — 2.6.0-alpha.0](CHANGELOG.md)
