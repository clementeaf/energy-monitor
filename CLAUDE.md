...
- **UX unificado + tests + DropdownSelect global:** `ChartSkeleton` en 6 páginas. 18 `<select>` nativos → `DropdownSelect` (0 restantes). `TableStateBody` en PlatformDashboard y DrilldownPage. Forms Modal → Drawer (Building, Meter). 8 tests UI nuevos (158 total). Buildings columna "Empresa" cross-tenant, Meters columna "Edificio". PlatformDashboard KPIs compactos, Globe Power excluido. Vite proxy → AWS. [CHANGELOG — 2.6.0-alpha.0](CHANGELOG.md)
