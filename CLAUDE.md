...
- **Chart skeletons + forms Drawer + cross-tenant UX:** `ChartSkeleton` compartido en 6 páginas. PlatformDashboard KPIs compactos (7 en fila), Globe Power excluido de KPIs. Buildings: columna "Empresa" en cross-tenant, form Drawer con selector empresa. Meters: columna "Edificio" sin filtro activo, form Drawer con DropdownSelect. Fix: `<label>` → `<div>` en forms (evita re-open DropdownSelect). Vite proxy → AWS backend. [CHANGELOG — 2.6.0-alpha.0](CHANGELOG.md)
