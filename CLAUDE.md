...
- **Globe Landing Figma pixel-perfect:** 11 secciones rediseñadas desde Figma MCP. Navbar (CTA sólido, fixed). Hero (598px, crossfade grid-stack, 3 slides dinámicos, logo bar 117px). About/Valores/Industrias/Diferenciación/Stats/Presencia/Innovación/Contacto/Footer — tipografía, spacing, gradients y colores exactos del Figma. Cards con 3 estados (default/hover/pressed) y gradient rojizo hover. Responsividad completa (mobile 1-col, font scaling, hover desktop-only). Scroll suave con offset navbar. [CHANGELOG — 2.3.0-alpha.0](CHANGELOG.md)
- **Prod deploy + cross-tenant + health:** Health endpoint público. super_admin ve buildings/meters cross-tenant. Refresh token lee `__Host-` cookie. Operator/Building switchers en sidebar para impersonation. RDS TLS en Docker. Globe Landing UI refresh. CloudFront `plataforma.globepower.cl` configurado. Lambda iot-ingest redeployed. RDS data 100% synced. [CHANGELOG — 2.2.0-alpha.0](CHANGELOG.md)
