...
export function InvitationAcceptPage() {
  ...
  useEffect(() => {
    ...
    validateInvitation(token)
      ...
      .catch(() => setValidationError('Invitación no encontrada o inválida.'));
  }, [token]);
      ...
      <div className="w-full max-w-sm rounded-2xl border border-pa-border bg-white p-8 shadow-sm">
        ...
        <div className="mb-6 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-pa-text-muted">Invitación de acceso</p>
          </div>
        </div>

        {/* Validation error */}
        {validationError && (
          <div className="mb-4 rounded-lg bg-red-50 px-3 py-3 text-center text-sm text-red-700">
            {validationError}
          </div>
        )}
          ...
          <>
            <div className="mb-5 rounded-lg bg-surface px-4 py-3">
              <p className="text-sm text-pa-text">
                Hola <span className="font-semibold">{invitation.name}</span>, has sido invitado como:
              </p>
              <p className="mt-1 text-sm font-semibold text-pa-navy">{invitation.roleLabel}</p>
              <p className="mt-1 text-xs text-pa-text-muted">{invitation.email}</p>
            </div>
            ...
            <p className="mb-3 text-center text-sm text-pa-text-muted">
              Inicia sesión para activar tu cuenta
            </p>
        ...
        {!invitation && !validationError && (
          <div className="flex justify-center py-6">
            <div className="h-6 w-6 animate-spin rounded-full border-2 border-pa-border border-t-pa-blue" />
          </div>
        )}
      </div>
    </div>
