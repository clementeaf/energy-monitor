#!/usr/bin/env node
/**
 * Aplica migración 018 (tablas de facturación) contra RDS.
 * Uso: node apply-018-billing.mjs
 *      Con túnel local: DB_HOST=127.0.0.1 DB_PORT=5433 node apply-018-billing.mjs
 */
...
const sqlPath = join(__dirname, '..', '..', 'sql', '018_billing_tables.sql');
...
async function main() {
  ...
  try {
    ...
    console.log('Applying sql/018_billing_tables.sql...');
    await client.query(sql);
    console.log('018_billing_tables.sql OK');
  } finally {
    await client.end();
  }
  console.log('Done.');
