    ...
    <aside className="flex w-52 shrink-0 flex-col bg-pa-bg-alt">
      <div className="px-4 pt-5 pb-4">
        <div className="flex items-center gap-2.5">
          {tenant?.logoUrl && (
            <img src={tenant.logoUrl} alt="Logo" className="h-8 w-8 shrink-0 object-contain" />
          )}
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 pt-2">
        ...
        {(visibleAdmin.length > 0 || hasAny('audit:read')) && (
          <>
            <div className="px-3 pb-1 pt-4 text-[11px] font-semibold uppercase tracking-wide text-pa-text-muted">
              Administración
            </div>
      ...
      <div className="border-t border-pa-border px-4 py-3">
        {user && (
          <div className="mb-2">
            <div className="truncate text-[13px] font-medium text-pa-text">{user.displayName ?? user.email}</div>
            <div className="text-[11px] text-pa-text-muted">{user.role.name}</div>
          </div>
        )}
        <button
          type="button"
      ...
      <button
        ...
        className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-[13px] font-medium transition-colors ${
          isActive
            ...
            : 'bg-white text-pa-text hover:bg-raised'
        }`}
      >
        <span className="flex items-center gap-2">
          {label}
          {alertBadge != null && alertBadge > 0 && (
            <span className="inline-flex size-5 items-center justify-center rounded-full bg-pa-coral text-[10px] font-bold text-white">
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center justify-between rounded-lg px-3 py-1.5 text-[13px] transition-colors ${
          isActive
            ? 'bg-pa-blue/10 font-medium text-pa-blue'
            : 'text-pa-text-muted hover:bg-pa-blue/5 hover:text-pa-text'
        }`
      }
    >
      {label}
      {badge != null && (
        <span className="ml-auto inline-flex size-5 items-center justify-center rounded-full bg-pa-coral text-[10px] font-bold text-white">
          {badge > 99 ? '99+' : badge}
        </span>
      )}
    </NavLink>
