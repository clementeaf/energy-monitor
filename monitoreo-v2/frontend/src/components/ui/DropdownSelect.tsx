...
interface DropdownSelectProps<T extends string = string> {
  ...
  disabled?: boolean;
  className?: string;
  /** Show a search input when options exceed this count (default 8) */
    ...
    <div ref={containerRef} className={`relative ${className}`} onKeyDown={onKeyDown}>
      <button
        ...
        className={
          ...
          'text-left transition-colors hover:border-gray-400 focus:border-[var(--color-primary,#3D3BF3)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary,#3D3BF3)] ' +
          (disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer')
        }
      >
        <span className={value ? 'text-gray-900' : 'text-gray-400'}>{selectedLabel}</span>
        <svg className={`ml-2 size-4 shrink-0 text-gray-400 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {showSearch && (
            <div className="border-b border-gray-100 p-2">
              <input
                ...
                placeholder="Buscar..."
                className="w-full rounded border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-300"
              />
            </div>
          )}
