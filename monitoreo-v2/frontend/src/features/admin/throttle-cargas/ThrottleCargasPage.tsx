  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto overflow-x-hidden">
      ...
      <div>
        ...
        <p className="text-xs text-muted">Control de consumo de API, throttling, quotas por tenant y gestión de extracciones</p>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="panel flex w-1/2 min-w-0 flex-col">
          <div className="shrink-0 px-4 pt-4 pb-2">
            <h3 className="text-sm font-semibold text-foreground">Quotas de API por tenant</h3>
            <p className="text-xs text-muted">alerta si un tenant supera el 80% de su cuota</p>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-left text-xs font-medium text-muted">
                ...
                <th className="sticky top-0 bg-background px-2 py-1.5">Actual</th>
                <th className="sticky top-0 bg-background px-2 py-1.5">% uso</th>
                <th className="sticky top-0 bg-background px-2 py-1.5">Estado</th>
              </tr>
            </thead>
                  ...
                  <td className="px-2 py-1.5">
                    <div className="flex items-center gap-1">
                      <div className="h-1.5 w-12 overflow-hidden rounded-full bg-surface">
                        <div className={`h-full rounded-full ${t.pct >= 80 ? 'bg-warning' : 'bg-brand'}`} style={{ width: `${t.pct}%` }} />
                      </div>
                      <span className={`font-mono ${t.pct >= 80 ? 'text-warning' : 'text-muted'}`}>{t.pct}%</span>
                    </div>
                  </td>
                  <td className="px-2 py-1.5">
                    <span className={`inline-block rounded-full px-1.5 py-0.5 text-xs font-medium ${t.estado === 'alerta' ? 'bg-warning/10 text-warning' : 'bg-success/10 text-success'}`}>
                      {t.estado}
                    </span>
                  </td>
                </tr>
        ...
        <div className="panel flex w-1/2 min-w-0 flex-col p-4">
          <div className="shrink-0">
            <h3 className="text-sm font-semibold text-foreground">Consumo de API por tenant</h3>
            <p className="mb-2 text-xs text-muted">requests/hora 24h · línea horizontal en el límite configurado</p>
            <div className="mb-2 flex items-center gap-3">
              <span className="flex items-center gap-1 text-xs text-muted">
                <span className="inline-block h-2 w-4 rounded bg-brand" /> PASA
              </span>
              <span className="flex items-center gap-1 text-xs text-muted">
                <span className="inline-block h-0.5 w-4 rounded bg-danger/60 border-dashed" /> umbral 5 000 req/h
              </span>
            </div>
          </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="panel min-w-0 flex-1 p-4">
          <h3 className="text-sm font-semibold text-foreground">Log de eventos de throttling</h3>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-left text-xs font-medium text-muted">
                ...
                <th className="sticky top-0 bg-background px-3 py-2">IP origen</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {THROTTLE_LOG.map((t, i) => (
                <tr key={i} className="animate-fade-in transition-colors hover:bg-surface" style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <td className="max-w-[120px] truncate px-3 py-2 font-mono text-xs text-muted" title={t.endpoint}>{t.endpoint}</td>
                  <td className="px-3 py-2 text-xs font-mono text-danger">{t.bloqueadas}</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted">{t.ip}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="panel min-w-0 flex-1 p-4">
          <h3 className="text-sm font-semibold text-foreground">Configurador de quotas</h3>
          <p className="mb-3 text-xs text-muted">whitelist de IPs exentas · cambios auditados (usuario/timestamp)</p>
          <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Tenant</label>
              <select className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" value={quotaForm.tenant} onChange={(e) => setQuotaForm({ ...quotaForm, tenant: e.target.value })}>
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="mb-1 block text-xs font-medium text-muted">Límite req/hora</label>
                <input className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" placeholder="ej. 5000" value={quotaForm.reqHora} onChange={(e) => setQuotaForm({ ...quotaForm, reqHora: e.target.value })} />
              </div>
              <div className="flex-1">
                <label className="mb-1 block text-xs font-medium text-muted">Límite req/día</label>
                <input className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" placeholder="ej. 50000" value={quotaForm.reqDia} onChange={(e) => setQuotaForm({ ...quotaForm, reqDia: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Whitelist de IPs exentas (ETL PASA)</label>
              <textarea className="w-full rounded border border-border bg-background px-2 py-1.5 text-xs text-foreground font-mono" rows={2} placeholder="10.0.1.45&#10;10.0.1.46" value={quotaForm.whitelist} onChange={(e) => setQuotaForm({ ...quotaForm, whitelist: e.target.value })} />
            </div>
            <div className="flex gap-2 pt-1">
              <button type="submit" className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-background hover:bg-brand/90">Guardar quota</button>
              <button type="button" className="rounded-md border border-border px-4 py-2 text-sm font-medium text-muted hover:bg-surface">Cancelar</button>
            </div>
          </form>
        </div>
      ...
      <div className="flex gap-3">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-left text-xs font-medium text-muted">
                ...
                <th className="sticky top-0 bg-background px-3 py-2">Últ. extracción</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {EXTRACTIONS.map((e, i) => (
                <tr key={e.integracion} className="animate-fade-in transition-colors hover:bg-surface" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-3 py-2 font-medium text-foreground">
                    <span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${e.estado === 'stalled' ? 'bg-danger' : 'bg-success/100'}`} />
                    {e.integracion}
                  </td>
                  <td className="px-3 py-2 text-xs text-muted">{e.modo}</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted">{e.cursor}</td>
                  <td className="px-3 py-2 text-xs text-muted">{e.ultExtraccion}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="panel min-w-0 flex-1 p-4">
          <h3 className="text-sm font-semibold text-foreground">Configurador de modo de extracción</h3>
          <p className="mb-3 text-xs text-muted">incremental / full-load · ventana de overlap para registros tardíos</p>
          <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Integración</label>
              <select className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" value={extractForm.integracion} onChange={(e) => setExtractForm({ ...extractForm, integracion: e.target.value })}>
                <option value="">Seleccionar…</option>
            ...
            <div>
              ...
              <select className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" value={extractForm.modo} onChange={(e) => setExtractForm({ ...extractForm, modo: e.target.value })}>
                <option value="incremental">Incremental</option>
                <option value="full-load">Full-load</option>
              </select>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Columna de cursor</label>
              <select className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" value={extractForm.cursor} onChange={(e) => setExtractForm({ ...extractForm, cursor: e.target.value })}>
                <option value="timestamp">timestamp</option>
                <option value="id">ID</option>
              </select>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted">Ventana de overlap</label>
              <div className="flex items-center gap-2">
                <input className="w-20 rounded border border-border bg-background px-2 py-1.5 text-sm text-foreground" placeholder="5" value={extractForm.overlap} onChange={(e) => setExtractForm({ ...extractForm, overlap: e.target.value })} />
                <span className="text-xs text-muted">min (ej. últimos 5 min)</span>
              </div>
            </div>
            <div className="flex gap-2 pt-1">
              <button type="submit" className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-background hover:bg-brand/90">Guardar</button>
              <button type="button" className="rounded-md border border-border px-4 py-2 text-sm font-medium text-muted hover:bg-surface">Cancelar</button>
            </div>
          </form>
        </div>
      ...
      <div className="panel p-4">
          <h3 className="text-sm font-semibold text-foreground">Historial de extracciones</h3>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-left text-xs font-medium text-muted">
                ...
                <th className="sticky top-0 bg-background px-3 py-2">Modo</th>
                <th className="sticky top-0 bg-background px-3 py-2">Registros</th>
                <th className="sticky top-0 bg-background px-3 py-2">MB</th>
            ...
            <tbody className="divide-y divide-border">
              {EXTRACT_HISTORY.map((h, i) => (
                <tr key={i} className="animate-fade-in transition-colors hover:bg-surface" style={{ animationDelay: `${i * 30}ms` }}>
                  <td className="px-3 py-2 font-mono text-xs text-muted">{h.inicio}</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted">{h.fin}</td>
                  <td className="px-3 py-2 text-xs text-muted">{h.modo}</td>
                  <td className="px-3 py-2 font-mono text-xs text-foreground">{Number(h.registros).toLocaleString()}</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted">{h.mb}</td>
                  <td className="px-3 py-2">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${h.resultado === 'éxito' ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>
                      {h.resultado}
                    </span>
                  </td>
                </tr>
