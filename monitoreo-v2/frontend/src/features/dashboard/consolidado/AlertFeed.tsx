...
export function AlertFeed({ alerts }: Readonly<{ alerts: Alert[] }>) {
  ...
  return (
    <div className="panel flex min-h-0 flex-1 flex-col">
      <h4 className="shrink-0 px-3 py-2 text-xs font-medium text-foreground">
        Alertas en vivo ({alerts.length})
      </h4>
      {alerts.length === 0 ? (
        <p className="px-3 py-4 text-xs text-muted">Sin alertas — operación normal.</p>
      ) : (
        <ul ref={listRef} className="min-h-0 flex-1 divide-y divide-border overflow-y-auto">
          {visible.map((a) => (
            <li key={a.id} className="flex items-start gap-2 px-3 py-2">
              <span className={`mt-0.5 inline-block shrink-0 rounded px-1.5 py-0.5 text-xs font-medium ${SEVERITY_COLORS[a.severity] ?? ''}`}>
          ...
          {visibleCount < alerts.length && (
            <li className="px-3 py-2 text-center text-xs text-muted">Cargando más...</li>
          )}
        </ul>
      )}
    </div>
