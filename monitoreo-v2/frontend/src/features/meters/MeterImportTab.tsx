...
import {
  ...
  useCommitMeterImport,
  useMeterImportJobsQuery,
  useMeterImportPreviewQuery,
  useValidateMeterImport,
} from '../../hooks/queries/useMeterImportQuery';
import type { MeterImportSummary, CommitMeterImportResponse } from '../../types/meter-import';
import { getFetchErrorMessage } from '../../lib/fetchError';
import { UserImportDropzone } from '../admin/users/UserImportDropzone';
import {
  METER_IMPORT_JOB_STATUS_LABELS,
  METER_IMPORT_STATUS_LABELS,
  formatMeterImportErrorCodes,
} from './meter-import-labels';

type ImportStep = 'upload' | 'preview' | 'done';

interface MeterImportTabProps {
  onViewMeters: () => void;
}
...
function RowStatusBadge({ status }: Readonly<{ status: string }>) {
  ...
  return (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${styles[status] ?? 'bg-surface text-muted'}`}>
      {METER_IMPORT_STATUS_LABELS[status] ?? status}
    </span>
  );
}

/**
 * Bulk meter import wizard: upload, preview, commit.
 */
export function MeterImportTab({ onViewMeters }: Readonly<MeterImportTabProps>) {
  const [step, setStep] = useState<ImportStep>('upload');
  const [jobId, setJobId] = useState<string | null>(null);
  const [summary, setSummary] = useState<MeterImportSummary | null>(null);
  const [filename, setFilename] = useState<string>('');
  const [rowsOffset, setRowsOffset] = useState(0);
  const [statusFilter, setStatusFilter] = useState<'valid' | 'error' | 'duplicate' | ''>('');
  const [commitResult, setCommitResult] = useState<CommitMeterImportResponse | null>(null);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [historyOpen, setHistoryOpen] = useState(false);

  const validateMutation = useValidateMeterImport();
  const commitMutation = useCommitMeterImport();
  const jobsQuery = useMeterImportJobsQuery();
  const rowsQuery = useMeterImportPreviewQuery(jobId, {
    limit: ROWS_PAGE,
    offset: rowsOffset,
    ...
    <div className="space-y-6">
        ...
        <section className="space-y-4">
          <div className="rounded-lg border border-border bg-surface/60 p-4 text-sm text-muted">
            ...
            <ul className="mt-2 list-inside list-disc space-y-1">
              <li><code className="text-xs">name</code> — nombre del medidor</li>
              <li><code className="text-xs">code</code> — código único por empresa</li>
              <li><code className="text-xs">building_code</code> o <code className="text-xs">external_site_id</code> — edificio</li>
            </ul>
            <p className="mt-2">
              Opcionales: meter_type, model, serial_number, phase_type (single_phase/three_phase),
              load_category (hvac, lighting, tenant, main, other), parent_meter_code, hierarchy_node_name,
              modbus_address, bus_id, uplink_route, external_id, is_active.
            </p>
            <button
              type="button"
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(['', 'valid', 'error', 'duplicate'] as const).map((value) => {
              const label = value === '' ? 'Todas' : METER_IMPORT_STATUS_LABELS[value];
          ...
          <div className="max-h-[50vh] overflow-y-auto panel">
            <table className="min-w-full divide-y divide-border text-sm">
              ...
              <TableStateBody
                ...
                skeletonWidths={['w-8', 'w-32', 'w-16', 'w-16', 'w-16', 'w-20', 'w-16', 'w-28']}
              >
                {(rowsQuery.data?.data ?? []).map((row) => (
                  <tr key={row.id} className="hover:bg-surface">
                    ...
                    <Td>{row.parentMeterCode ?? '—'}</Td>
                    <Td>{row.hierarchyNodeName ?? '—'}</Td>
                    <Td><RowStatusBadge status={row.status} /></Td>
                    <Td className="max-w-xs truncate text-xs text-muted" title={formatMeterImportErrorCodes(row.errorCodes)}>
                      {formatMeterImportErrorCodes(row.errorCodes)}
                    </Td>
                  </tr>
                ))}
              </TableStateBody>
      ...
      {step === 'done' && commitResult ? (
        <section className="rounded-lg border border-green-200 bg-green-50/80 p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={onViewMeters}>Ver medidores</Button>
                  ...
                  <tbody>
                    {jobsQuery.data?.data.map((job) => (
                      <tr key={job.id} className="border-t border-border">
                        ...
                        <td className="py-2 pr-4">{METER_IMPORT_JOB_STATUS_LABELS[job.status] ?? job.status}</td>
