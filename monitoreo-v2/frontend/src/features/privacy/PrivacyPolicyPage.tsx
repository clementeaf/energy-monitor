  ...
  return (
    <div className="mx-auto max-w-3xl py-8">
      <PageHeader
        ...
        eyebrow="Legal"
        description={policy ? `Versión ${policy.version} — Vigente desde ${policy.effectiveDate}` : undefined}
      />

      {query.isPending && <p className="mt-6 text-sm text-subtle">Cargando...</p>}
        ...
        <div className="mt-8 space-y-8">
          ...
          {policy.sections.map((section, i) => (
            <div key={i}>
              ...
              {section.content && (
                <p className="mt-2 text-sm text-foreground">{section.content}</p>
              )}
              {section.items && (
