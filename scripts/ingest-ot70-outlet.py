#!/usr/bin/env python3
"""
Full ingestion of Arauco Premium Outlet Buenaventura (OT70) into pg-arauco.

Steps:
  1. CSV → raw_readings (~2.45M rows, UTF-8 BOM, comma decimals)
  2. CSV → store (70 meters with real store_types)
  3. raw_readings → meter_monthly (aggregated)
  4. raw_readings → meter_readings (70 partitions)
  5. XLSX Resumen Mensual → meter_monthly_billing
  6. XLSX Resumen Mensual → meter_monthly_billing KPIs (update)
  7. XLSX Pliegos Tarifarios (Quilicura) → tariff (new location)
  8. XLSX Resumen Mensual → building_summary
  9. meter_monthly → building_summary KPIs (with 1-year offset)

Usage: python3 scripts/ingest-ot70-outlet.py
"""
...
# --- Config ---
CSV_PATH = 'docs/outlet/OUTLET_70_anual.csv'
XLSX_PATH = 'docs/outlet/OUTLET_KPIs_mensuales_2025_M.xlsx'
BUILDING = 'Arauco Premium Outlet Buenaventura'
YEAR = 2025
METER_PREFIX = 'OT'
METER_COUNT = 70
BATCH_SIZE = 5000
...
cur.execute("SELECT COUNT(*) FROM raw_readings WHERE meter_id LIKE 'OT-%'")
existing = cur.fetchone()[0]
if existing > 0:
    print(f'  Ya existen {existing:,} filas OT en raw_readings, saltando...')
else:
    start = time.time()
    total = 0
    batch = []

    # NOTE: CSV is UTF-8 BOM (not latin1 like MG/MM)
...
cur.execute("SELECT COUNT(*) FROM raw_readings WHERE meter_id LIKE 'OT-%'")
print(f'  Verificación raw_readings OT: {cur.fetchone()[0]:,}')


# =============================================
# STEP 2: CSV → store (70 meters with real store_types)
# =============================================
print('\n' + '=' * 60)
print('STEP 2: CSV → store (70 medidores)')
print('=' * 60)
...
cur.execute("DELETE FROM meter_monthly WHERE meter_id LIKE 'OT-%'")
print(f'  meter_monthly OT eliminados previos: {cur.rowcount}')
conn.commit()
...
cur.execute("""
    ...
    FROM meter_monthly WHERE meter_id LIKE 'OT-%'
""")
r = cur.fetchone()
...
records = []
for row in rows_data[3:]:  # skip header rows (1: title, 2: subtitle, 3: column headers)
    mes_name = row[0]
...
# =============================================
# STEP 7: XLSX Pliegos Tarifarios (Quilicura) → tariff
# =============================================
print('\n' + '=' * 60)
print('STEP 7: XLSX → tariff (Quilicura)')
print('=' * 60)

ws_tariff = wb['Pliegos Tarifarios (Quilicura)']
...
cur.execute("DELETE FROM tariff WHERE location = 'Quilicura'")
print(f'  Eliminados previos Quilicura: {cur.rowcount}')
...
conn.commit()
print(f'  Tariffs Quilicura insertados: {len(tariff_records)}')
...
# =============================================
print('\n' + '=' * 60)
print(f'STEP 8: building_summary ({BUILDING})')
print('=' * 60)
...
for row in summary_data:
    ...
    summary_records.append((
        BUILDING, month,
        METER_COUNT,    # total_stores
        0,              # store_types
        METER_COUNT,    # total_meters
        METER_COUNT,    # assigned_meters
        0,              # unassigned_meters
        total_kwh,
        peak,
        demanda,
...
# =============================================
# STEP 9: building_summary KPIs (from meter_monthly)
# =============================================
print('\n' + '=' * 60)
print('STEP 9: building_summary KPIs (CRITICAL)')
print('=' * 60)

# meter_monthly uses 2026 (synthetic CSV), building_summary uses 2025 (real XLSX)
# Offset: bs.month = mm.month - INTERVAL '1 year'
SQL_KPI_SUMMARY = """
...
cur.execute(SQL_KPI_SUMMARY, (BUILDING,))
updated_kpi = cur.rowcount
conn.commit()
print(f'  building_summary KPIs actualizados: {updated_kpi}')

# Verify no NULLs
cur.execute("""
    SELECT COUNT(*) AS total, COUNT(peak_demand_kw) AS has_peak,
           COUNT(avg_power_kw) AS has_avg, COUNT(avg_power_factor) AS has_pf
    FROM building_summary WHERE building_name = %s
""", (BUILDING,))
v = cur.fetchone()
print(f'  Verificación KPIs: {v[0]} meses, peak={v[1]}, avg_kw={v[2]}, avg_pf={v[3]}')
if v[0] != v[1] or v[0] != v[2] or v[0] != v[3]:
    print('  ⚠ ALERTA: hay KPIs NULL en building_summary!')
else:
    print('  ✓ Todos los KPIs poblados correctamente')
...
checks = [
    ("raw_readings OT", "SELECT COUNT(*) FROM raw_readings WHERE meter_id LIKE 'OT-%'"),
    ("store OT", "SELECT COUNT(*) FROM store WHERE meter_id LIKE 'OT-%'"),
    ("store total", "SELECT COUNT(*) FROM store"),
    ("store_type total", "SELECT COUNT(*) FROM store_type"),
    ("meter_monthly OT", "SELECT COUNT(*) FROM meter_monthly WHERE meter_id LIKE 'OT-%'"),
    ("meter_monthly total", "SELECT COUNT(*) FROM meter_monthly"),
    ("meter_readings OT", "SELECT COUNT(*) FROM meter_readings WHERE meter_id LIKE 'OT-%'"),
    ("meter_readings total", "SELECT COUNT(*) FROM meter_readings"),
    ("billing OT", f"SELECT COUNT(*) FROM meter_monthly_billing WHERE building_name = '{BUILDING}'"),
    ("billing total", "SELECT COUNT(*) FROM meter_monthly_billing"),
    ("building_summary OT", f"SELECT COUNT(*) FROM building_summary WHERE building_name = '{BUILDING}'"),
